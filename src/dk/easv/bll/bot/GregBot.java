...
public class GregBot implements IBot{
    ...
    private Node selectPromisingNode(Node rootNode)
    {
        ...
        {
            List<Node> promisingChildren = new ArrayList();

            //creating list of child that havent been visited yet
            for(Node child : promisingNode.getChildren())
            {
                if(child.getNumberOfVisits() == 0)
                {
                    promisingChildren.add(child);
                }
            }

            //if some of the children havent been visited - return random child from one of these children
            if(!promisingChildren.isEmpty())
            {
                promisingNode = promisingChildren.get(rand.nextInt(promisingChildren.size()));
            }

            //if all the children have been already visited - return random child
            else
            {
